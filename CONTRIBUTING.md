...
There are phases when UI5 Tooling has already started developing a new major version of it's projects but does not has released the new major version. This sitatuation requires a special handling of pull requests / commits if the actual coding change should be part of the current (e.g. Version 3) and the new major version (e.g. Version 4).
