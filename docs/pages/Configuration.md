...
There are two ways to define the set of components preload bundles should be generated for: `paths` (allowing patterns) and `namespaces`. You can also combine both configuration options. Defining any of them overrides the default preload bundle generation for the root component of `application` projects.
        ...
        builder:
          componentPreload:
            namespaces:
              ...
              - "my/awesome/app/componentTwo"
            excludes:
              - "my/awesome/app/componentOne/thirdparty/**"
              - "my/awesome/app/componentTwo/thirdparty/**"
              - "!my/awesome/app/componentTwo/thirdparty/NotExcluded.js"
        ```

The `excludes` option allows to define resources which shall not be added to the component preload bundles.

Patterns are always applied relative to the project's virtual source directory `/resources/`. Note that re-includes must start with the namespace of the component they apply to.
...
The `excludes` option allows to define resources which shall not be added to the `library-preload.js` bundle.

Patterns are always applied relative to the project's virtual source directory `/resources/`. Note that re-includes must start with library's namespace.
...
- Adds support for `exclude` configuration of [component](#excludes)- and [library](#excludes_1) preload bundles
